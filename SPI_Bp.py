...
M_star_dot = 0.1#10.23 # For GJ 436 (R_star=0.464 R_sun), this results in n_sw_base ~ 1e7 cm^-3 
for indi in star_array:
	...
	#for ind1 in Bp0_arr:
		npoints=4000
		Bp0_arr=np.linspace(0, 4, npoints, endpoint=True)
		print(r_orb/R_star)
		...
		ax2.set_xlabel(r"Planet magnetic field [G]",fontsize=20)
		#ax1.set_ylabel(r"${\rm log} (M_A)$")
		ax2.set_ylabel(r"${\rm log}$ (Flux density [mJy])",fontsize=20)
		xpos=(xmax+xmin)/2
		
		ypos=np.log10(3*rms)+0.0.5#+0.15
		...
		if open_field:
			...
			outfile = str(Exoplanet.replace(" ", "_"))+'/'+ Exoplanet + "-Open-Bstar"+ common_string+'_B_pl_variation'
		else:
			#ax1.text(x=0, y=1, s= Exoplanet + " - Closed field")
			outfile = str(Exoplanet.replace(" ", "_"))+'/'+ Exoplanet + "-Closed-Bstar"+ common_string+'_B_pl_variation'
