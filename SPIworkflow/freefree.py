...
def get_gaunt(T,nu):
    ...
    return g
'''
def get_taunu(nu,T,g,dist_absorption,n_sw):
    knu=3.692*10**8*(1-np.exp((-h*nu)/(k_B*T)))*Z**2*g*T**(-1/2)*nu**(-3)
    alphanu = knu*n_sw**2
    taunu=int.trapezoid(alphanu,dist_absorption)
    return knu,alphanu,taunu
'''

def ff_absorption(M_star,nu,T,m_av,mdot,inner_limit,outer_limit,Nsteps):
    '''
    ...
    inner_limit: altitude over the stellar surface where SPI emission takes place, in cm
    outer_limit: distance where the free-free absorption would become irrelevant, in cm
    Nsteps: number of points for the distance array
    '''
    
    dist_absorption = np.linspace(inner_limit, outer_limit, Nsteps)
