...
print(data)
print(data.columns)
print(data['planet_name'][18])
print(data['star_name'][18])
print(data['d_star(pc)'][18])
print(data['radius_star(r_sun)'][18])
print(data['mass_star(m_sun)'][18])

print(data['p_rot(days)'][18])
print(data['bfield_star(gauss)'][18])
print(data['radius_planet(r_earth)'][18])

print(data['mass_planet(m_earth)'][18])
print(data['p_orb(days)'][18])
print(data['a(au)'][18])
print(data['freq_cycl(ghz)'][18])
print(data.iloc[18])
df1 = pd.DataFrame(data)

#print(df1[['planet_name', 'star_name', 'd_star(pc)', 'radius_star(r_sun)',
#       'mass_star(m_sun)', 'p_rot(days)', 'bfield_star(gauss)',
#       'radius_planet(r_earth)', 'mass_planet(m_earth)', 'p_orb(days)',
#       'a(au)', 'freq_cycl(ghz)']][1])
#print(data['planet_name'][18])
print(data[:])
...
for indi in star_array:
	...
	for ind in Bfield_geom_arr:
		for ind1 in Bp0_arr:
			print('geometry: ',Bfield_geom_arr_descr[ind])
			print('Planet magnetic field: ',Bp0_arr[ind1])
			print(r_orb/R_star)
			...
			print('Omega_planet :',Omega_planet)
			#
			...
			print('lambertw k=-1',lambertw(-D_r[0], k=-1))
			print('lambertw k=0',lambertw(-D_r[0], k=0))
			for i in range(len(d_orb)):
				print(d_orb,r_orb)
				if (d_orb[i]/r_sonic) >= 1.0:
					#print('Supra-alfvenic')
					print('r_orb>r_sonic')
			...
			v_sw_base = -vsound**2 * lambertw(-D_base, k=0)    # Stellar wind velocity at the closest distance to the star
			print("v_sw_base: ",v_sw_base)
			#M_star_dot_all=[0.1,1,5,10,50]
			npoints=400
			#M_star_dot_all=np.logspace(-1.52, 1.48, num=npoints, base=10)
			#M_star_dot_all=np.logspace(-4.52, 4.48, num=npoints, base=10)
			M_star_dot_all=np.logspace(-10, 10, num=npoints, base=10)
			...
			if Bp0:
				...
				print('Bp (B_Earth): ',Bp); print('type: ',type(Bp))
			else:
			...
			print(ymin,ymax)
			#ax1.set_ylim([ymin_ax1,ymax_ax1])#ax1.set_ylim([ymin_ax1,0])
			#ax1.set_ylim([ymin_ax1,0])#ax1.set_ylim([ymin_ax1,0])
			ax1.set_ylim([-3,0.2])#ax1.set_ylim([ymin_ax1,0])
			ax2.set_ylim([ymin,ymax])
			
			#ax2.fill_between(range(len(y)), ymin, ymax, where=(y >0) , alpha=0.5)
			plt.axvline(x =alfven_mass_loss_limit , color = 'red', label = 'axvline - full height',linewidth=1)
			ax2.axvline(x =alfven_mass_loss_limit , color = 'red', label = 'axvline - full height',linewidth=1)
			
			
			plt.axvspan(alfven_mass_loss_limit, xmax, color='red', alpha=0.25, lw=0)
			...
			if alfven_mass_loss_limit<xmax :
				if xmax-alfven_mass_loss_limit>alfven_mass_loss_limit-xmin:
					ax2.text(x=alfven_mass_loss_limit+0.05,y=ymin+(ymax-ymin)*0.9,s="SUPER-ALFVÉN REGIME",fontsize=18,color='red')
				
			ypos=(ymax+ymin)/2
			ax2.text(x=xpos+(xmax-xpos)/2+0.5*xpos,y=ypos-(ymax-ypos)/1.5,s=r"$B_{pl}="+str(float(Bp[0]))+"$ G",fontsize='medium',color='black')
			...
			ax2.text(x=(xmax+2*xmin)/2,y=(2.5*ymax+ymin)/2,s='Saur/Turnpenney',color='orange',fontsize='x-small')
			ax2.text(x=(xmax+2*xmin)/2,y=(2.3*ymax+ymin)/2,s='Zarka/Lanza',color='blue',fontsize='x-small')
			#ax2.text(x=xpos,y=ypos*0.75,s=r"$B_pl="+str(Bp[0])+"$ G",fontsize='medium',color='black')
			...
			#if not os.path.exists(str(Exoplanet)):
			#	os.system('mkdir '+str(Exoplanet))
			...
			if open_field:
				...
				outfile = str(Exoplanet.replace(" ", "_"))+'/'+ Exoplanet + "-Open-Bstar"+ common_string+'_M_dot_variation'
			else:
				#ax1.text(x=0, y=1, s= Exoplanet + " - Closed field")
				outfile = str(Exoplanet.replace(" ", "_"))+'/'+ Exoplanet + "-Closed-Bstar"+ common_string+'_M_dot_variation'
