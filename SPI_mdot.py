...
#Bfield_geom_arr = [0, 1]
Bfield_geom_arr = [0, 1]
Bfield_geom_arr_descr=['closed','open']
...
mdot_min=-1
mdot_max=1
M_star_dot_all_definition=np.logspace(mdot_min, mdot_max, num=npoints, base=10)#M_star_dot_all_definition=np.logspace(-2.52, 2.48, num=npoints, base=10)
print('M_star_dot_all_definition :',M_star_dot_all_definition)
for indi in star_array:
	...
	for ind in Bfield_geom_arr:
		for ind1 in Bp0_arr:
			...
			v_sw2_base = -vsound**2 * lambertw(-D_base, k=0)    # Stellar wind velocity at the closest distance to the star
			v_sw_base=np.sqrt(v_sw2_base)
			print("v_sw_base: ",v_sw_base)
			#M_star_dot_all=[0.1,1,5,10,50]
			#npoints=4
			#
			M_star_dot_all=np.copy(M_star_dot_all_definition)
			#M_star_dot_all=np.logspace(-1.52, 1.48, num=npoints, base=10)
			print("M_star_dot_all: ",M_star_dot_all)
			#for M_star_dot in M_star_dot_all:
			# Plasma number density at base of the corona
			n_base_corona=[]
			for M_star_dot in M_star_dot_all:
				basewind=spi.n_wind(M_star_dot, R_star, v_sw_base, mu)
			...
			if Bp0:
				...
				Bp = np.ones(len(d_orb))*magfield_planet  # For now, force Bp=1.0 Gauss
			else:
			...
			#M_star_dot_all=np.logspace(-1.52, 1.48, num=npoints, base=10)
			M_star_dot_all=np.copy(M_star_dot_all_definition)
			x=np.log10(M_star_dot_all)
			y=np.log10(M_A)
			y_diff=abs(y)
			log_M_A_0=np.where(y_diff == y_diff.min())
			print(log_M_A_0,x[log_M_A_0])
			print(log_M_A_0,y[log_M_A_0])
			alfven_mass_loss_limit=x[log_M_A_0]
			print('alfven_mass_loss_limit: ',alfven_mass_loss_limit)
			...
			print(ymin,ymax)
			ax1.set_ylim([ymin_ax1,1])#ax1.set_ylim([ymin_ax1,0])
			...
			#
			#ax11.set_xlabel("Orbital period [days]",fontsize=20)
			#ax1.set_xlabel(r"log(Mass Loss rate [$\dot{M}_\odot$])",fontsize=20)
			ax2.set_xlabel(r"log(Mass Loss rate [$\dot{M}_\odot$])",fontsize=20)
			ax1.set_ylabel(r"${\rm log} (M_A)$",fontsize=20)
			ax2.set_ylabel(r"${\rm log}$ (Flux density [mJy])",fontsize=20)
			#ax2.axhline(y=np.log10(3*0.01), color='r', linestyle='--')
			ax2.axhline(y=np.log10(3*0.03), color='black', linestyle='--')
			xpos=(xmax)/2
			ypos=np.log10(0.1)+0.15
			ax1.axvspan(alfven_mass_loss_limit, xmax, color='red', alpha=0.25, lw=0)
			ax1.axvline(x =alfven_mass_loss_limit , color = 'red', label = 'axvline - full height',linewidth=1)
			ax2.axvspan(alfven_mass_loss_limit, xmax, color='red', alpha=0.25, lw=0)
			ax2.axvline(x =alfven_mass_loss_limit , color = 'red', label = 'axvline - full height',linewidth=1)			
			#ax2.text(x=xpos,y=ypos,s=r"$100 \mu$ Jy",fontsize='medium',color='black')
			#ax2.text(x=xpos,y=ypos,s=r"3 times 30 uJy",fontsize='medium',color='black')
			#ypos=np.log10(3*0.01)
			#ax2.text(x=xpos,y=ypos,s=r"3 times 10 uJy",fontsize='medium', color='r')
			#plt.axhline(y=np.log10(0.080), color='r', linestyle='-')
			...
			#ax2.text(x=-1,y=-3.5,s='Saur/Turnpenney',color='orange',fontsize='x-large')
			#ax2.text(x=-1,y=-3.7,s='Zarka/Lanza',color='blue',fontsize='x-large')
			#ax2.set_ylim([min(min(y_min_ZL),min(y_min)), max(max(y_max_ZL),max(y_max))])
			#ax2.set_ylim([-2.5,1.5])
			ax2.set_ylim([-2,2])
			#ax2.set_ylim([-4.5,0])
			
			#print('limits :',([min(min(y_min_ZL),min(y_min)), max(max(y_max_ZL),max(y_max))]))
			#ax2.set_ylim([min(min(y_min_ZL),min(y_min)), 1.5])
			
			ax1.set_facecolor("white")	
			ax2.set_facecolor("white")	
			#ax1.xaxis.set_tick_params(labeltop=True)
			#ax1 = ax1.secondary_xaxis('top')
			#ax1.xaxis.set_tick_params(labelbottom=True,top=True)
			...
			#ax2 = ax2.secondary_xaxis('top')
			
			
			ax1.xaxis.set_tick_params(labeltop=True,top=True)
			ax1.xaxis.set_tick_params(labelbottom=False,bottom=True)
			ax1.tick_params(axis='x', which='minor',top=True)
			...
			ax2.xaxis.set_tick_params(labeltop=False,top=True)
			...
			#more_lines = ['limits :'+str(min(min(y_min_ZL),min(y_min)))+','+str(max(max(y_max_ZL),max(y_max)))++'geometry='+str(open_field)]
			#with open('readme.txt', 'a') as f:
			#	f.write('\n'.join(more_lines))
    
			#with open('readme.txt', 'w') as f:
    			#	f.write('limits :'+str(min(min(y_min_ZL),min(y_min)))+','+str(max(max(y_max_ZL),max(y_max)))++'geometry='+str(open_field))
    				
    			#	,([min(min(y_min_ZL),min(y_min)), max(max(y_max_ZL),max(y_max))]))
			#ax1.set_xticklabels(x, fontsize=20)
			#ax2.tick_params(axis='x', fontsize=20)
			#ax2.tick_params(axis='y', fontsize=20)
			#ax1.tick_params(axis='y', fontsize=20)
			if str(Exoplanet.replace(" ", "_"))=='GJ_486_b':
				ax1.axvline(x=np.log10(1.4020249747030762), color='gray', linestyle='-')
				ax1.set_ylim([-1,0])
				ax2.axvline(x=np.log10(1.4020249747030762), color='gray', linestyle='-')
				ax2.set_ylim([-2.5,1.5])
			...
			try:
				...
				pass
			common_string = str(B_star)+"G"+"-Bpl"+str(Bp[loc_pl])+"G"+'_'+str(eps_min*100)+'-'+str(eps_max*100)+'perc'

			if open_field:
				#ax1.text(x=0, y=1, s= Exoplanet + " - Open field")
				outfile = str(Exoplanet.replace(" ", "_"))+'/'+ Exoplanet + "-Open-Bstar"+ common_string+'_M_dot_'+'T_corona'+"{:.2e}".format(T_corona)
			else:
				#ax1.text(x=0, y=1, s= Exoplanet + " - Closed field")
				outfile = str(Exoplanet.replace(" ", "_"))+'/'+ Exoplanet + "-Closed-Bstar"+ common_string+'_M_dot_'+'T_corona'+"{:.2e}".format(T_corona)
			...
			with open(outfileTXT, 'w') as f:
				...
				#print('n_base_corona','nu_plasma_corona')
				...
				#n_base_corona=' '.join(str("%.2f" % e) for e in n_base_corona)
				#nu_plasma_corona=' '.join(str("%.2f" % e) for e in nu_plasma_corona)
				length = len(n_base_corona)  # Get the length of the list
				if length % 2 != 0:  # Check if the length is odd
					middle_index = length // 2
					#return lst[middle_index]
				else :#If the length is even
					middle_index = length // 2 - 1
				nu_plasma_corona=nu_plasma_corona/(10**6)
				f.write('mass loss rate = ['+ str("%.2f" % M_star_dot_all_definition[0])+'; '+str("%.2f" % M_star_dot_all_definition[middle_index])+'; '+ str("%.2f" % M_star_dot_all_definition[-1])+']\n')
				#f.write('n_base_corona (cm^-3) = ['+ str("%.2f" % n_base_corona[0])+'; '+str("%.2f" % n_base_corona[middle_index])+'; '+ str("%.2f" % n_base_corona[-1])+']\n')
				f.write('n_base_corona (cm^-3) = ['+ str("{0:.2E}".format(np.real(n_base_corona[0])))+'; '+str("{0:.2E}".format(np.real(n_base_corona[middle_index])))+'; '+ str("{0:.2E}".format(np.real(n_base_corona[-1])))+']\n')
				f.write('nu_plasma_corona (MHz) = ['+ str("%.2f" % nu_plasma_corona[0])+'; '+str("%.2f" % nu_plasma_corona[middle_index])+'; '+ str("%.2f" % nu_plasma_corona[-1])+']\n')
				#n_base_corona)
				#f.write('nu_plasma_corona (MHz) = ' +nu_plasma_corona)
				...
				f.write('alfven_mass_loss_limit: {0} \n'.format(10**alfven_mass_loss_limit))
