...
#ax4.plot(x, P_B_sw*np.ones(len(x))/(8*np.pi), color='b', linestyle='dashdot')
#ax4.plot(x, P_dyn_sw*np.ones(len(x))/(8*np.pi), color='r', linestyle='solid')
#ax4.plot(x, P_th_sw*np.ones(len(x))/(8*np.pi), color='g', linestyle=(0,(1,3.5)))
# LPM - CHECK IF USEFUL
...
ax1.axvline(x = 4.5, ls='-', color='gray', lw=2)
ax2.axvline(x = 4.5, ls='-', color='gray', lw=2)
ax3.axvline(x = 4.5, ls='-', color='gray', lw=2)
ax4.axvline(x = 4.5, ls='-', color='gray', lw=2)

def identity(x):
    return x
    
secax = ax1.secondary_yaxis('right', functions=(identity,identity))
secax = ax2.secondary_yaxis('right', functions=(identity,identity))
secax = ax3.secondary_yaxis('right', functions=(identity,identity))
secax = ax4.secondary_yaxis('right', functions=(identity,identity))
